...
class GotoMenu(urwid.ListBox):
    ...
    def __init__(self, user_timelines):
        ...
        self.server_edit = EditBox(caption="Server URL: ", edit_text="https://")
        ...
        def _server(local):
            ...
            if base_url:  # and re.match(base_url, r"http[s]*\S+"):  # sanity check
                ...
                try:
                    ...
                    test_generator = api.anon_public_timeline_generator(instance=instance["uri"], local=True, limit=1)
                    self.message_widget.set_text(("cyan_bold", "Checking server..."))
                    result = next(test_generator)
                    if (result):
                        self._emit("public_timeline", True, instance)
                    else:
                        self.message_widget.set_text(("warning", f"Error getting timeline from {base_url}"))
                except Exception:
                    self.message_widget.set_text(("warning", f"Error getting timeline from {base_url}"))
            else:
                self.message_widget.set_text(("warning", "Server URL required"))
